...
def shard(args: argparse.Namespace) -> None:
    spec = _spec(args.learner_library_version); family = args.family; signal = tune_boundary_signal(family, args.delta)[0]
    ...
    frame = run_independent_grid(replications=args.count, replicate_start=args.start, sample_size=300, inference_folds=5, delta=args.delta, critical_value=critical, vimp_spec=spec, seed=args.seed, scenario_families=(family,), boundary_signals={family: signal}, workers=workers)
    args.output.parent.mkdir(parents=True, exist_ok=True); frame.assign(delta_target=args.delta).to_parquet(args.output, index=False)
...
def training_manifest(args: argparse.Namespace) -> None:
    ...
    deltas = sorted(set(data.delta_target)) if "delta_target" in data else []
    if len(deltas) != 1:
        raise ValueError(f"shards must share one delta_target; found {deltas}")
    ...
    payload = {"critical_value": float(critical), "critical_quantile": CRITICAL_QUANTILE, "delta_target": float(deltas[0]), "distribution_file": distribution.name, "distribution_sha256": file_sha256(distribution), "successful_training": eligible.groupby("scenario_family").size().astype(int).to_dict(), "vimp_fingerprint": vimp_fingerprint(_spec(args.learner_library_version))}
...
def main() -> None:
    ...
    shard_parser = commands.add_parser("shard"); shard_parser.add_argument("--phase", choices=["training", "validation"], required=True); shard_parser.add_argument("--family", choices=REQUIRED_SCENARIO_FAMILIES, required=True); shard_parser.add_argument("--start", type=int, required=True); shard_parser.add_argument("--count", type=int, required=True); shard_parser.add_argument("--output", type=Path, required=True); shard_parser.add_argument("--delta", type=float, default=.05); shard_parser.add_argument("--seed", type=int, default=20260809); shard_parser.add_argument("--learner-library-version", default="v3_nested_blend"); shard_parser.add_argument("--workers", type=int, help="parallel replications; defaults to the core count"); shard_parser.add_argument("--training-manifest"); shard_parser.set_defaults(func=shard)
