...
def valid_record(spec: VimpSpec) -> CalibrationRecord:
    ...
    return CalibrationRecord(profile_id="fixture", scenario_family=families[0], delta_target=.05, inference_rows=300, inference_folds=5, vimp_fingerprint=vimp_fingerprint(spec), critical_value=-2, distribution_file="fixture.parquet", distribution_sha256="fixture", calibration_replications=5000, evaluation_replications=5000, empirical_rejection_rate=.04, upper_rejection_bound=.049, validation_scenario_families=families, validation_replications_per_family={family: 5000 for family in families}, grid_upper_rejection_bounds={family: .049 for family in families}, status="validated")
