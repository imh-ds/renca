...
def write_edge_report(graph: ResolutionGraph, estimates: list[VimpEstimate], output_dir: str | Path, eligibility: list[CalibrationEligibility] | None = None) -> tuple[Path, Path]:
    ...
    for pair in graph.pairs:
        ...
        rows.append({"pair_id": pair.pair_id, "state": pair.state.value, "separator": ",".join(pair.separator), "adjusted_p": pair.adjusted_p, "causal_status": pair.causal_status, "vimp_statuses": ";".join(estimate.status for estimate in directions), "calibration_statuses": ";".join(estimate.calibration_status for estimate in directions), "eligibility_status": ";".join(sorted({item.status for item in pair_eligibility})), "eligibility_mismatch_fields": ";".join(sorted({field for item in pair_eligibility for field in item.mismatch_fields})), "theta_estimates": ";".join(str(estimate.theta_hat) for estimate in directions), "resolution_reason": payload["reason"]})
