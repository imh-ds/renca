...
def validate_grid(results: pd.DataFrame, *, profile_id: str, scenario_family: str, delta_target: float, inference_rows: int, inference_folds: int, vimp_fingerprint: str, critical_value: float, distribution_file: str = "", distribution_sha256: str = "", calibration_replications: int | None = None, calibration_successful_replications_per_family: dict[str, int] | None = None, required_scenario_families: tuple[str, ...] = REQUIRED_SCENARIO_FAMILIES, maximum_ineligibility_rate: float = .05, alpha: float = .05) -> CalibrationRecord:
